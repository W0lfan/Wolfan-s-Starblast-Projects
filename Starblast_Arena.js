...
var map = {
	...
	map_ids : [
		2739,2742,2817,2865,2900,2961,4989,4955,4897,4797
	],
	crystal_values: [
		1,1.1,1.2
	],
...
this.options = {
  root_mode: " ",
  survival_level: 8,
  crystal_value: map.crystal_values[~~(Math.random()*map.crystal_values.length)],
  ships: ships,
  map_size: 100,
  reset_tree:true, 
  map_id: map.map_ids[~~(Math.random()*map.map_ids.length)],
  crystal_drop: 1.1,
  starting_ship: 800,
  starting_ship_maxed: true,
  boucing_lasers: false,
  map_name: map.names[~~(Math.random()*map.names.length)],
  vocabulary: map.vocabulary,
  max_players: 30,
  release_crystal:true,
  weapons_store: false,
  friendly_colors:2,
  soundtrack: map.soundtracks[~~(Math.random()*map.soundtracks.length)],
...
var limitations = [
	70, // Aliens limitation
	50  // Asteroids limitation
];
var aliens = [
	[10,11,17,18], // Codes
	[0,1,2,3] 	   // Types
...
var factionInit = function(game) {
  if (game.step === 0) {
    for (let i = 0;i<2;i++) {
    	var name = teams.names[i][~~(Math.random()*teams.names[i])];
    	var place = teams.names.indexOf(name)
    	var hue = teams.colors_hues[i][place]
    	var game_hue = teams.game_hues[i][place]
    	teams[i].color_hue = hue;
    	teams[i].game_hue = game_hue;
    	teams[i].name = name;
    }
  };
};
...
var add_AsteroidsAndAliens = function(game) {
	if (game.aliens.length <= maxAliensLimit) {
		game.addAlien({
			x : (Math.random() - 0.5) * game.options.map_size * 10,
			y : (Math.random() - 0.5) * game.options.map_size * 10,
			code: aliens[0][randomNumber(0,5)],
			level : aliens[1][randomNumber(0,5)],
			crystal_drop : randomNumber(50,60)
		})
	}if (game.asteroids.length <= maxAsteroidsLimit ) {
		game.addAsteroid({
			y: (Math.random() - 0.5) * game.options.map_size * 10,
			x: (Math.random() - 0.5) * game.options.map_size * 10,
			size: randomNumber(50,70),
			vx: randomNumber(0.1,0.9),
			vy: randomNumber(0.1,0.9)})
	}
};
var random_ship = function(ship, isRandom =null) {
	if (isRandom) {
		ship.set({
			type: isRandom,
		})
		...
		ship.set({
			type: ships_types[~~(Math.random()*ships_types.length)]
		})
	}
	ship.set({
		generator: 1000,
...
var doWinner = function(game) {
	for (let i = 0;i<2;i++) {
		if (teams[i].score.aliens >= aliensKills && teams[i].score.ships >= shipsKills) {
			for (let ship of game.ships) {
				...
				var all_quotes = {
					...
					" " : " ",
					[quotes[0]] : teams[0].score.ships,
					[quotes[1]] : teams[0].score.aliens,
					[quotes[2]] : teams[0].score.aliens + teams[0].score.ships,
					[quotes[3]] : teams[1].score.ships,
					[quotes[4]] : teams[1].score.aliens,
					[quotes[5]] : teams[1].score.aliens + teams[1].score.ships,
				}
				if (ship.team == i) {
					ship.gameover({
...
var team_assignation = function(ship) {
	if (ship.custom.team === null) {
		if (teams[0].players_number === teams[1].players_number) {
			ship.set({team: randomNumber(0,2)});
		} else if (teams[0].players_number > teams[1].players_number) {
			ship.set({team:0});
		} else if (teams[0].players_number < teams[1].players_number) {
			ship.set({team:1});
		}
		teams[ship.team].players_number++;
		ship.custom.team = teams[ship.team].name;
		if (ship.team === 0) {
			ship.custom.ennemies = teams[1];
		}else {
			ship.custom.ennemies = teams[0];
		}
		ship.custom.ennemies = teams[ship.team ]
		ship.set({
			hue: teams[ship.team].game_hue,
			x: (Math.random() - 0.5) * game.options.map_size * 10,
			y: (Math.random() - 0.5) * game.options.map_size * 10
		});
...
if (game.custom.scoreboard_init !== true) {
	for (let i = 0; i < 90;i++) {
		scoreboard.components.push(
			{ type: "text",position:[72.5,i + 4,25,12],value: "",color:"#CDE",align:"right"},
			{ type: "player",position:[2.5,i + 5,65,10],id:"",color:"#CDE",align:"left"}
		)
	}
	scoreboard.components.push(
	...
	game.custom.scoreboard_init = true;
};
...
var shipInit = function(game) {
	for (let ship of game.ships) {
	  if (ship.custom.init !== true) {
		...
		team_assignation(ship);
		setPannels(ship);
		aliensKills+=points.aliens;
		shipsKills+=points.ships;
	  }
	}
  };
...
var hidePannelComponents = function(ship) {
  ...
  shipComponents.forEach(element => {
		ship.setUIComponent({
			id: element, visible:false
		})
	});
}
var timerShips = {
...
var pointsSettingIfJoins = function(game) {
  ...
  pointsShow.components[0].value = `Your objective is to kill ${set1} aliens and ${set2} ships`
  for (let ship of game.ships) {
...
var pointsShow = {
     id: "pointsShow",
     position: [25,-10,60,70],
     visible: true,
     components: [
         { type: "text",position:[12.5,0,60,50],
          value:``,
          color:"#CDE"},
     ]
   };
...
var showYOURPoints = {
     ...
     components: [
         { type: "text",position:[12.5,0,60,50],
          value:"Press [P] to see your points",
          color:"#CDE"},
     ]
   };
...
var assignPointsToTeam = function(ship, points) {
  if (ship.team === 0) {
    teams[0].score.ships += points;
    teams[0].score.aliens += points;
  } else if (ship.team === 1) {
    teams[0].score.ships += points;
    teams[0].score.aliens += points;
  }
...
this.event = function(event, game) {
  ...
  switch (event.name) {
    case "ui_component_clicked":
      ...
      var ships_components = ["ship1","ship2","ship3","ship4","ship5","ship6"];
      for (let component_ of ships_components) {
        if (component === component_) {
          if (ship.custom.pannelType === 1) {
            ship.set({
              type: 600 + ships_components.indexOf(component_) + 1 , stats:66666666,shield:1000,generator:0
            });
          } else {
            ship.set({
              type: 600 + ships_components.indexOf(component_) + 1 + 6, stats:66666666,shield:1000,generator:0
            });
          }
        }
      };
    ...
    case "alien_destroyed":
      if (alien !== null){
        if (killer !== null) {
          ...
          killer.set({points:killer.custom.points})
          for (let i in codes[0]) {
            for (let i_ in codes[1]) {
              if (alien.code === i && alien.level === i_) {
                assignPointsToTeam(killer,codes[2][codes.indexOf(i)]);
                return;
              }
            }
          }
    ...
    case "ship_destroyed":
      if (ship !== null) {
        if (killer !== null) {
          teams[killer.team].points.ships++;
          killer.set({points:killer.custom.points})
        }
        ship.custom.deaths++;
      }
